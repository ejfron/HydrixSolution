<!-- components/user/PasscodeSetup.vue -->
<template>
      ...
      <Transition name="scale">
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
          <div class="bg-gradient-to-r from-green-600 to-emerald-600 px-6 py-5">
            ...
            <h2 class="text-xl font-black text-white text-center">Setup Passcode</h2>
            <p class="text-green-100 text-xs text-center mt-1">Create a passcode to protect Workers section</p>
          </div>

          <div class="p-6 space-y-4">
            <div v-if="!isSettingUp">
              <p class="text-sm text-gray-600 mb-4 text-center">
                You need to set up a passcode first before accessing the Workers section.
              </p>
              <button 
                @click="isSettingUp = true"
                class="w-full bg-green-600 hover:bg-green-700 text-white py-3 rounded-2xl font-bold text-sm transition cursor-pointer"
              >
                Set Up Passcode Now
              </button>
              <button 
                @click="handleCancel"
                class="w-full mt-3 border border-gray-200 hover:bg-gray-50 text-gray-700 py-3 rounded-2xl font-semibold text-sm transition cursor-pointer"
              >
                Go Back
              </button>
            </div>
...
const handleNext = () => {
  ...
  if (step.value === 1) {
    tempPasscode.value = passcodeInput.value
    step.value = 2
    passcodeInput.value = ''
    error.value = ''
  } else {
    if (passcodeInput.value === tempPasscode.value) {
      // Emit the passcode to the parent
      emit('success', passcodeInput.value)
      resetForm()
    } else {
      error.value = 'Passcodes do not match. Please try again.'
      passcodeInput.value = ''
    }
  }
}
