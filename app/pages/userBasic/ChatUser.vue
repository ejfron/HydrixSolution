      ...
      <div class="p-4 sm:p-8 space-y-6">
        ...
        <div v-if="userPlan === 'basic'">
          <div class="bg-gradient-to-r from-yellow-50 to-yellow-100 border border-yellow-200 rounded-2xl p-6 text-center">
            <h3 class="font-bold text-gray-800 mb-2">Subscription Required</h3>
            <p class="text-yellow-700 mb-4">
              This feature requires Standard or Premium Subscription to Access. 
              Please Upgrade Your Subscription to Access This Feature.
            </p>
            <button 
              @click="router.push('/userBasic/Subscription')"
              class="px-6 py-2 bg-green-600 hover:bg-green-700 text-white rounded-xl font-semibold transition"
            >
              Upgrade Now
            </button>
          </div>
        ...
        <!-- Show full chat feature for Standard and Premium users -->
        <div v-else>
          <!-- Header -->
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              ...
              <p class="text-slate-500 text-xs sm:text-sm mt-1">
                Single Chat Box - Create up to 2 users
                <span v-if="userPlan === 'premium'" class="ml-2 text-xs bg-gradient-to-r from-purple-500 to-pink-500 text-white px-2 py-0.5 rounded-full">
                  Premium
                </span>
                <span v-else-if="userPlan === 'standard'" class="ml-2 text-xs bg-blue-500 text-white px-2 py-0.5 rounded-full">
                  Standard
                </span>
              </p>
            </div>
...
// Subscription and user data
const userPlan = ref<'basic' | 'standard' | 'premium'>('basic')
...
const fetchUserPlan = async () => {
  try {
    ...
    if (user) {
      ...
      const profile: any = resp.data

      // Set the plan based on your database schema
      // Adjust this based on your actual column names
      userPlan.value = profile?.subscription_plan || profile?.plan || 'basic'
    }
  } catch (error) {
    console.error('Error fetching user plan:', error)
    userPlan.value = 'basic' // Default to basic if error
  } finally {
