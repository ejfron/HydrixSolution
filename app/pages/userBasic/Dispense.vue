    ...
    <main class="flex-1 min-w-0">
              ...
              <div class="p-6 space-y-4 overflow-y-auto flex-1">
                ...
                <div>
                  ...
                  <div v-if="riders.length === 0" class="px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-slate-400 text-xs">
                    <Truck :size="14" class="inline mr-1" />
                    No riders added yet.
                  ...
                  <div v-else class="space-y-2">
                    <button @click="selectedRiderId = null; selectedRiderName = ''; walkInPrice = null"
                      ...
                      <div :class="['w-8 h-8 rounded-xl flex items-center justify-center text-sm shrink-0', !selectedRiderId ? 'bg-green-600' : 'bg-slate-100']">
                ...
                <div class="border-t border-slate-100 pt-4">
                  ...
                  <div v-if="paymentStatus === 'unpaid'" class="bg-red-50 rounded-xl px-3 py-2 border border-red-100">
                    <p class="text-xs text-red-600 font-semibold flex items-center gap-1">
                      <AlertCircle :size="14" />
                      Buong ₱{{ totalAmount.toFixed(2) }} ay utang — walang binayad
                    </p>
                  </div>
                  ...
                  <div v-if="paymentStatus === 'partial'" class="mb-3">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">
                      ...
                      Amount Paid Now <span class="text-red-400">*</span>
                    </label>
                    <div class="relative">
                      <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 text-sm font-bold">₱</span>
                      <input v-model.number="partialAmount" type="number" min="1" :max="totalAmount - 1"
                        placeholder="e.g. 50"
                  ...
                  <div v-if="paymentStatus === 'paid'" class="bg-green-50 rounded-xl px-3 py-2 border border-green-100">
                    <p class="text-xs text-green-700 font-semibold flex items-center gap-1">
                      <CheckCircle :size="14" />
                      Fully paid — ₱{{ totalAmount.toFixed(2) }}
                    </p>
                  </div>
                </div>
            ...
            <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden max-h-[90vh] overflow-y-auto">
              ...
              <div class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">
                      <Ruler :size="14" class="inline mr-1" />
                      Unit
                    </label>
      ...
      <div class="p-4 sm:p-8 space-y-6 sm:space-y-8">
        ...
        <div v-if="gallonTypes.length === 0" class="bg-white rounded-3xl border border-dashed border-slate-300 p-12 text-center">
          ...
          <button @click="showCreateModal = true" class="px-6 py-3 bg-green-600 hover:bg-green-700 text-white rounded-2xl font-bold text-sm transition cursor-pointer flex items-center justify-center gap-2 mx-auto">
            <Plus :size="16" />
            Create First Gallon
          </button>
        </div>
...
// REMOVED: stats ref (no longer needed)
...
const totalAmount = computed(() => {
  if (selectedType.value === 'reseller') return resellerTotal.value
  return regularTotal.value
})
...
const amountPaidNow = computed(() => {
  ...
  if (paymentStatus.value === 'partial') return partialAmount.value || 0
  return 0
})
...
// REMOVED: fetchSales function (no longer needed)
...
const dispense = async () => {
  ...
  if (paymentStatus.value === 'partial') {
    if (!partialAmount.value || partialAmount.value <= 0) {
      errorMsg.value = 'Please enter the partial payment amount.'
      return
    }
    if (partialAmount.value >= totalAmount.value) {
      errorMsg.value = 'Partial payment must be less than the total. Use "Paid" instead.'
      return
    }
  }
