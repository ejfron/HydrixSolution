  ...
  <div class="min-h-screen bg-[#f5f7fb] flex">
    ...
    <main class="flex-1 min-w-0">
      ...
      <div class="p-4 sm:p-8 space-y-6">
        <!-- Premium Plan Badge -->
        <div class="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-xl p-3 flex items-center justify-between">
          <div class="flex items-center gap-2">
            <div class="w-8 h-8 rounded-full bg-gradient-to-r from-green-600 to-emerald-600 flex items-center justify-center">
              <span class="text-white text-sm font-bold">P</span>
            </div>
            <div>
              <p class="text-sm font-semibold text-green-800">Premium Plan</p>
              <p class="text-xs text-green-600">Unlimited users for chatting</p>
            </div>
          </div>
          <button 
            @click="router.push('/userPremium/Subscription')"
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p class="text-slate-500 text-xs sm:text-sm mt-1">Unlimited Chat Users - Create as many users as you want</p>
          </div>
        </div>

        <!-- Create Users Section (unlimited for premium) -->
        <div class="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl p-6 border border-green-100">
          <div class="flex items-center gap-2 mb-3">
            <UserPlus :size="20" class="text-green-600" />
            <h3 class="font-semibold text-gray-800">Create New User ({{ users.length }} users total)</h3>
          </div>
          <p class="text-sm text-slate-600 mb-4">Create unlimited users who can chat with each other</p>
          <div class="flex gap-3">
            <input
              v-model="newUserName"
              @keyup.enter="createUser"
              type="text"
              placeholder="Enter user name (e.g., Boss, Cherry, Admin)"
        ...
        <!-- User Selection Tabs (scrollable for many users) -->
            ...
            <div class="p-4 border-b border-slate-100 bg-gradient-to-r from-green-50 to-emerald-50">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <h3 class="font-semibold text-gray-800">Group Chat</h3>
                    <p class="text-xs text-slate-500">{{ users.length }} participants • {{ messages.length }} messages</p>
                  </div>
                </div>
                <div class="flex gap-2">
                  <button 
                    @click="clearAllMessages"
                    class="text-xs text-slate-500 hover:text-red-600 transition px-2 py-1"
                  >
                    Clear Chat
                  </button>
                </div>
        ...
        <div v-else-if="users.length === 1" class="bg-amber-50 border border-amber-200 rounded-2xl p-8 text-center">
          ...
          <h3 class="font-semibold text-gray-800 mb-1">Need at least 2 users to chat</h3>
          <p class="text-sm text-slate-600">Create one more user to start chatting</p>
        </div>

        <!-- Premium Features Info Card -->
        <div class="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-2xl p-4">
          <div class="flex items-start gap-2">
            ...
            <div>
              <p class="text-sm font-semibold text-green-800">Premium Features:</p>
              <ul class="text-xs text-green-700 mt-1 space-y-1">
                <li>• ✅ Unlimited users (no restrictions)</li>
                <li>• ✅ Clear chat history option</li>
                <li>• ✅ All messages saved per plan</li>
                <li>• ✅ Switch between any user instantly</li>
...
// PREMIUM PLAN STORAGE KEYS (DIFFERENT FROM STANDARD)
const STORAGE_KEY_USERS = 'chat_users_premium'
const STORAGE_KEY_MESSAGES = 'chat_messages_premium'
...
const checkSubscription = async () => {
  try {
    ...
    console.log('Premium Chat page plan check:', userPlan)
...
const createUser = (): void => {
  ...
  if (!name) return // No limit for premium!
  
  // Updated green theme colors - all green variations for premium
  const colors: string[] = [
